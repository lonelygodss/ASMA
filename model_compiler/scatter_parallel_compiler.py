...
class ScatterParallelCompiler(CompilerBase):
    ...
    def _divide_gate_proj(self, function: Function, compiled_model: CompiledModel):
        ...
        self.h_divisions = (output_dim + self.array_h - 1) // self.array_h
        self.v_divisions = (input_dim + self.array_v - 1) // self.array_v
        ...
        for j in range(self.h_divisions):
            ...
            for i in range(self.v_divisions):
                ...
                compute_j = j + 1
                output_tensor_id = self._create_tensor_id(base_coords, i=compute_i, j=compute_j)
                add_input_tensors.append((output_tensor_id, {'size_h': slice_h/self.v_divisions, 'size_v': 1}))
            for i in range(self.v_divisions):
                # Create output tensor ID for addition
                compute_index = j*self.v_divisions+i
                add_output_tensor_id = self._create_tensor_id(base_coords, i=i+1+self.v_divisions, j=add_j)
                output_size = {'size_h': slice_h/self.v_divisions, 'size_v': 1}
    ...
    def _divide_up_proj(self, function: Function, compiled_model: CompiledModel):
        ...
        for j in range(self.h_divisions):
            ...
            for i in range(self.v_divisions):
                # Create output tensor ID for addition
                compute_index = j*self.v_divisions+i
                add_output_tensor_id = self._create_tensor_id(base_coords, i=1+i+self.v_divisions, j=add_j)
                output_size = {'size_h': slice_h/self.v_divisions, 'size_v': 1}
    ...
    def _divide_glu(self, function: Function, compiled_model: CompiledModel):
        ...
        for i in range(self.v_divisions):
            for j in range(self.h_divisions):
                ...
                compute_index = j*self.v_divisions+i
                start_h = (compute_index) * self.array_h/self.v_divisions
                end_h = min((compute_index + 1) * self.array_h/self.v_divisions, output_dim)
                ...
                output_tensor_id = self._create_tensor_id(base_coords, i=element_idx, j=compute_index+1)
                ...
                add_subfunction_coords['i'] = -2
                add_subfunction_coords['j'] = 1+compute_index
                ...
                activation_coords['j'] = compute_index+1
    ...
    def _divide_activation(self, function: Function, compiled_model: CompiledModel):
        ...
        for i in range(self.v_divisions):
            for j in range(self.h_divisions):
                ...
                add_subfunction_coords = base_coords.copy()
                add_subfunction_coords['i'] = -2
                add_subfunction_coords['j'] = compute_index+1
